...
@Component({
  selector: 'app-checkinhome',
  templateUrl: './checkinhome.component.html',
  styleUrls: ['./checkinhome.component.scss']
})
export class CheckinhomeComponent implements OnInit {
  ...
  onClick() {
    this.http.get(this.url + "/" + this.gID).subscribe(
      ...
      data => {
        ...
        this._ngZone.run(() =>
          this.router.navigate(["/checkin/profile"], {
      ...
      error => {
        ...
        this._ngZone.run(() => this.router.navigate(['/checkin/reg'], {relativeTo: this.activatedRoute.parent}));
      }
    );
  }
  ...
  profile() {
    this.router.navigateByUrl("/checkin/profile");
  }
