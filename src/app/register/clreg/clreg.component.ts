...
@Component({
  selector: 'app-clreg',
  templateUrl: './clreg.component.html',
  styleUrls: ['./clreg.component.scss']
})
export class ClregComponent implements OnInit {
    ...
    .subscribe(result =>
      {
        ...
        this._ngZone.run(() => this.router.navigate(['/upgrade/clregform'],{relativeTo: this.activatedRoute.parent}));
  ...
  ngOnInit() {
    ...
    // }else{
      if(this.uid==undefined){
        this._ngZone.run(() => this.router.navigate(['/cl'],{relativeTo: this.activatedRoute.parent}));
      }else{
